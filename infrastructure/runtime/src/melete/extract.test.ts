...
describe("extractFromMessages", () => {
  it("parses JSON from LLM response", async () => {
    const json = JSON.stringify({
      facts: ["Widget torque spec is 185 ft-lbs per service manual"],
      decisions: ["Decision: use polymer leather for belt due to durability"],
    ...
    expect(result.decisions).toEqual(["Decision: use polymer leather for belt due to durability"]);
...
describe("noise filtering", () => {
  it("filters generic patterns from extraction output", async () => {
    ...
    // Should keep: Project Alpha, Prosoche, Qdrant migration

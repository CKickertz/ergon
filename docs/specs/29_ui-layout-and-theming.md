...
5. **Light mode doesn't exist.** The theme toggle in Settings switches `data-theme` to `"light"` but `:root` has no `[data-theme="light"]` override block. Selecting "Light" produces an unstyled mess. The Ardent site (`acme-workshop.example.com`) uses a warm light palette (`#F7F3E8` background, `#2A2725` text, same dye accents) that should be the reference.
...
**Reference:** Acme Workshop site. Warm parchment, not cold white. Same dye accents, same font families, inverted luminance.

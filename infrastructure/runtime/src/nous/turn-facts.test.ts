...
describe("extractTurnFacts", () => {
  it("extracts facts from a substantive response", async () => {
    const router = mockRouter('["Widget torque spec is 185 ft-lbs", "Decision: use polymer leather for belt project"]');
    const result = await extractTurnFacts(
      router,
      "Based on the manual, the widget torque spec is 185 ft-lbs. I also decided to go with polymer leather for the belt project because of its durability advantages over veg-tan for this particular use case. The grain structure holds up better under daily wear.",
      "",
      "test-model",
    );
    expect(result.facts).toHaveLength(2);
    expect(result.facts[0]).toContain("185 ft-lbs");
    expect(result.facts[1]).toContain("polymer");
  });
  ...
  it("includes tool summary in extraction context", async () => {
    const router = mockRouter('["Steering box replacement requires 185 ft-lbs torque on the widget"]');
    await extractTurnFacts(
      router,
      "x".repeat(200),
      'exec(grep "pitman" manual.txt) → Widget: 185 ft-lbs',
      "test-model",
    );

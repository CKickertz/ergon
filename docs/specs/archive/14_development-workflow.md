...
- How agents identify themselves as contributors vs. Alice owning authorship

The result: every Claude Code task produces a slightly different workflow, and cleanup falls on Alice or me.
...
2. **Alice owns the git history.** All commits use Alice's author identity. Agents are tooling, not contributors. The log should read like one person built this — because one person directed it.
...
Agents already use this — formalize it. No `Co-authored-by` trailers, no agent attribution in commits. The AGENTS.md and PR descriptions can note which agent did the work, but the git log is Alice's.
...
- Commit with: git config user.name "Alice Johnson" && git config user.email "alice@example.com"  
...
5. Alice approves or Syn merges with Alice's standing approval for spec work
...
1. Alice or Syn decides "this is a release" after a meaningful batch of work
...
This eliminates the deploy bottleneck: Alice toggles a setting, the system pulls the update. No CLI, no Claude Code in the loop. Implementation details belong in Spec 03 Phase 2 — this spec establishes the versioning that makes it possible.
...
3. **Release cadence.** Intentional releases at major milestones, not per-push automation. Hotfixes get sub-versions. The real goal: a beta/bleeding-edge toggle in the webchat UI that lets Alice switch between stable releases and latest main without CLI or Claude Code involvement.
...
Agents aren't "tools" — but they're also not human. `Co-authored-by: Claude` on every commit is misleading (Syn isn't simply an Anthropic model) and reduces credibility for a public repo. Alice owns authorship for now. This will evolve as the ecosystem matures and agent identity has a better public framing.
